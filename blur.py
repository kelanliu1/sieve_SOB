...
def blur_objects(images: sieve.Image, tracked_objects: Dict, class_to_blur: str, object_masks: List) -> sieve.Image:
    import numpy as np
    import cv2
    import uuid
    ...
    for i in range(len(images_by_frame_number)):
        ...
        objects_in_frame = tracked_objects.get(i, [])
        ...
        yield sieve.Image(path=new_path, frame_number=image.frame_number, fps=image.fps)
